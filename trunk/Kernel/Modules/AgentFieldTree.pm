...
sub Run {
    ...
    if ( $Self->{Subaction} eq 'ChildsJSON' ) {
       ...
       my $JSON = $LayoutObject->JSON(
	    ...
	    #my $ReadOnly = $ParamObject->GetParam( Param => "ReadOnly" ) || 0;
		...
		for my $Field ( sort { $a->{Position} <=> $b->{Position} } @FieldList ) {
			...
			if ($Display && $Field) 
			{
				my $Data = {
	            	%{$Par},
	                %{$Field},
	                Value => $ValueSet->{$FieldID}||$ParamObject->GetParam( Param => $ParamObject->GetParam( Param => "DynamicFieldName" ) . '_Field_' . $FieldID ),
		            ...
		            FieldPrefix => $ParamObject->GetParam( Param => "DynamicFieldName" )
	            };
