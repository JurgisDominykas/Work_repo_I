...
Core.App = (function (TargetNS) {
    ...
    TargetNS.FinallyList = [];

    /**
     * @function
     *      This function collects all callbacks to be executed after all elements and files of this page are loaded and all actions triggered by Core.App.Ready event are done..
     * @param {Function} Callback The callback function to be executed
     * @return nothing
     */

    TargetNS.Finally = function(Callback) {
        if ($.isFunction(Callback)) {
            var Trace;
            try {
                TargetNS.FinallyList[TargetNS.FinallyList.length] = Callback;
            }
            catch (Error) {
    ...
    //        Core.Exception.ShowError('No function parameter given in Core.App.Finally', 'TypeError');
        }
    };

    /**
     * @function
     *      This function executes all the callbacks collected by Core.App.Finally event..
     * @param nothing
     * @return nothing
     */

    TargetNS.ExecuteFinally = function() {
        for(var FinalEvent in TargetNS.FinallyList)
            TargetNS.FinallyList[FinalEvent]();
    }
