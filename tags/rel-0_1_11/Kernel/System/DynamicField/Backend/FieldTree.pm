...
#    $Self->{ParamObject} = Kernel::System::Param->new( %{$Self} );
...
sub ValueSet {
    ...
    if($Param{Value}->{ValueSetID} eq "NEW")
    {
    	...
    	$Self->{LogObject}->Log(
    		'Priority' => 'error',
    		'Message' => 'FieldTree::ValueSet; ValueSetId = ' . $ValueSetID 
    	);
    }
...
sub EditFieldRender {
    ...
    if ( defined $FieldValue->{ValueSetID} ) {
        $Value = $FieldValue;
    }
		  ...
		  my %GetParam;
			...
			my @FieldTreeStructure = $Self->{FieldTreeObject}->JSON(
				...
				else
				{
				    ...
				    $Self->{'LogObject'}->Log(
				        'Priority' => 'error',
				        'Message'  => "ItemId = " . $Param{FieldTreeID},
				    );
