...
sub Run {
    ...
    if ( $Self->{Subaction} eq 'ChildsJSON' ) {
       ...
       my $JSON = $Self->{LayoutObject}->JSON(
        ...
        my $DoNotShowWarnings = $Self->{ParamObject}->GetParam( Param => "DoNotShowWarnings" ) || 0;
		my $DynamicFieldPrefix = $Self->{ParamObject}->GetParam( Param => "DynamicFieldName" );
		...
		for my $Field ( sort { $a->{Position} <=> $b->{Position} } @FieldList ) {
			...
			if ($Display && $Field) 
			{
				my $Data = {
	            	%{$Par},
	                %{$Field},
	                Value => $ValueSet->{$FieldID}||$Self->{ParamObject}->GetParam( Param => $Self->{ParamObject}->GetParam( Param => "DynamicFieldName" ) . '_Field_' . $FieldID ),
