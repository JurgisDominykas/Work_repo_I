    ...
    if ($Param{ParamObject}) {
        ...
        if ($Value->{ValueSetID} eq "NEW")
        {
            ...
            $LayoutObject->Block(
                ...
                Data => {
                    %Param,
                   		FieldTreeRecursiveIDs => ($FieldTreeIDFromPost =~ m/^\d+$/) ? $Self->{FieldTreeObject}->FieldTreesRecursiveIDs(
                    	ItemID => $FieldTreeIDFromPost,
                    	Separator => ',',
                    	RootID => $Self->{ConfigObject}->Get("FieldTree::ProblemTypeRootFieldTreeID")
                    ) : '',
                    ValueSetID => $Value->{ValueSetID},
            ...
            for my $Row (@FieldTreeStructure) {
                ...
                for my $RowLevel2 (@{ $Row->{items} }) {
                    ...
                    my $FieldIDListLevel3 = $Self->{FieldTreeObject}->FieldList(
                        FieldTreeID => $RowLevel2->{id},
                        Valid => 1,
                    );
                    
                    if (scalar @{ $RowLevel2->{items} } == 0 && ( keys %{$FieldIDListLevel3} ) == 0)
                    {
                        $LayoutObject->Block(
                            Name => 'EmptyBranch',
                            Data => {},
...
sub ReadableValueRender {
    ...
    my $Content = "";
    $Content .= "Pasirinkta kategorija: " . $FieldTreeName . "<br/> <br/>";
