    ...
    foreach my $FieldID ( sort keys %{$Data}) {
    	...
    	my $FieldDBType = $FieldDBTypes->{$Field->{FieldType}};
	        ...
	        if (($Field->{FieldType} eq "Select") && ($Data->{$FieldID} =~ /^-?\d+$/)) {
	            ...
	            $Self->{LogObject}->Log(
                Priority => 'notice',
                Message  => "\%\%$Field->{Name}\%\%$Data->{$FieldID}",
            );
...
sub ValueSetGetFieldTreeID {
    ...
    my $Field;
    while ( my @Row = $Self->{DBObject}->FetchrowArray() ) {
	    $Self->{Cache}->{ValueSetGetFieldTreeID}->{FieldTreeID}->{ $Param{ValueSetID} } = return $Row[0];;
        return $Row[0];
    }
...
sub RulesPostProcess {
    ...
    # check needed stuff
    for my $Argument (qw(TicketID TicketData)) {
        if ( !$Param{$Argument} ) {
            $Self->{LogObject}->Log(
                Priority => 'error',
                Message  => "Need $Argument!",
	...
	my $ValueSet = $Self->ValueSetGet(
		ValueSetID => $TicketData->{ProblemDescriptionFieldsetID},
	);
    ...
    foreach my $RuleID ( sort keys %{$Rules} ) {
    	...
    	if ($Rule->{FieldType} eq 'RuleRedmineProjectID') {
            #$Rule->{Template}; # FIXME FIXME: implement this
        }
    }
